...
lb_full = [0.005 * ones(1,7), 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, 0.02, -120, 50];
ub_full = [0.080 * ones(1,7), 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, 0.9, -25, 180];

% Create a handle that locks the mode to 'full'
eval_full = @(x) run_blade_physics(x, params_base, 'full');

[x_opt_full, hist_full] = damped_bfgs(phys_simp, @blade_objective, @blade_constraints, x0_full, lb_full, ub_full);

% =========================================================================
% PLOT: Full model
% =========================================================================
disp('--- Plotting Optimized Full Geometry ---');

% 1. Reconstruct the parameter structure cleanly using our new function
params_full_plot = update_blade_params(x_opt_full, params_base, 'full');

disp(['Optimized thickness (t): ', num2str(x_opt_full(1))])
disp(['Optimized Radius 4 (R4): ', num2str(x_opt_full(2))])
...
% =========================================================================
disp('======================================================');
disp('--- Starting MATLAB Genetic Algorithm ---');
...
options_ga = optimoptions('ga', ...
    ...
    'UseParallel', false, ...
    'PlotFcn', @gaplotbestf);       % Set to true if you want to use multiple CPU cores
