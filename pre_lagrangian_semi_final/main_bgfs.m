...
% New ploting call
plot_blade_segments(bx_simp, by_simp, params_simp_plot.res, 'Simple geometry BFGS', save_dir, save_op)
plot_blade_thickness(struct_mesh_simp, 'Simple geometry BFGS', save_dir, save_op);
plot_def_stress(results_simp, struct_mesh_simp, params_simp_plot, stress_data_simp, 'Simple geometry BFGS', save_dir, save_op);
...
plot_blade_segments(bx_full, by_full, params_full_plot.res, 'Full geometry BFGS', save_dir, save_op)

% Compute properties and plot deformation/stress
struct_mesh_full = material_properties([bx_full; by_full], params_full_plot);
plot_blade_thickness(struct_mesh_full, 'Full geometry BFGS', save_dir, save_op);
...
plot_def_stress(results_full, struct_mesh_full, params_full_plot, stress_data_full, 'Full geometry BFGS', save_dir, save_op);
...
options_ga = optimoptions('ga', ...
    'Display', 'iter', ...
    'PopulationSize', 100, ...    % Smaller population for quicker testing
...
[x_opt_h, hist_h] = damped_bfgs(eval_h, @blade_objective, @blade_constraints, x0_h, lb_full, ub_full);
time_hybrid = toc;
