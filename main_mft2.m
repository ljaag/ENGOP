...
% --- Run Phase 1: Simplified Model ---
% x0 = [thickness, Radius]
x0_simp = [0.010, 0.15];
lb_simp = [0.002, 0.05]; % Min thickness 2mm, Min radius 5cm
ub_simp = [0.030, 1.0]; % Max thickness 30mm, Max radius 40cm

% Create an anonymous function handle to pass params_base
obj_simp = @(x) simplified_blade_wrapper(x, params_base);

disp('Starting Simplified Optimization...');
tic
%[x_opt_simp, hist_simp] = custom_penalty_gradient_descent(obj_simp, x0_simp, lb_simp, ub_simp);
[x_opt_simp, hist_simp] = custom_optimizer(obj_simp, x0_simp, lb_simp, ub_simp);
toc
fprintf('Optimized Thickness: %.4f m\n', x_opt_simp(1));
fprintf('Optimized Radius: %.4f m\n', x_opt_simp(2));