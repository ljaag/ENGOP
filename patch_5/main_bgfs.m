...
% ---------------------------------------------------------
% PHASE 3: Run the Native Genetic Algorithm (GA)
% ---------------------------------------------------------
disp('======================================================');
disp('--- Starting MATLAB Genetic Algorithm (GA) ---');

% We use the same bounds and initial guess from Phase 1
nvars = length(x0_simp);

% 1. Setup the wrapper handles (Ensure ga_objective, ga_nonlcon, and
%    ga_evaluator_cache are saved in your folder!)
obj_fun_ga = @(x) ga_objective(x, params_base, 'simplified');
nonlcon_fun_ga = @(x) ga_nonlcon(x, params_base, 'simplified');

% 2. Configure GA Options
% You can tune these values for your report to discuss performance
options_ga = optimoptions('ga', ...
    'Display', 'iter', ...
    'PopulationSize', 50, ...    % Smaller population for quicker testing
    'MaxGenerations', 50, ...    % Max iterations
    'InitialPopulationMatrix', x0_simp, ... % Seed it with our BFGS starting guess
    'UseParallel', true);       % Set to true if you want to use multiple CPU cores
